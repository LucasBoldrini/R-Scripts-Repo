...
fixed_costs <- 10e6 # Total costs for full program (arbitrary)
...
demand <- function(price) {
  ...
  intercept <- 2000 # Maximum number of teams that would participate if the competition were free (different for full program) - Arbitrary number
  slope <- -0.30 # How the expected number of teams changes with the registration price - Arbitrary number
